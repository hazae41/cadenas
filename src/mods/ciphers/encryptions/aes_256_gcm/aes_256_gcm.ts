...
export class AES_256_GCM {
  ...
  static async tryInit(secrets: Secrets): Promise<Result<AES_256_GCM, CryptoError>> {
    return await Result.unthrow(async t => {
      const encryption = await tryCrypto(async () => {
        return await crypto.subtle.importKey("raw", secrets.client_write_key, { name: "AES-GCM", length: 256 }, false, ["encrypt"])
      }).then(r => r.throw(t))

      const decryption = await tryCrypto(async () => {
        return await crypto.subtle.importKey("raw", secrets.server_write_key, { name: "AES-GCM", length: 256 }, false, ["decrypt"])
      }).then(r => r.throw(t))

      return new Ok(new AES_256_GCM(secrets, encryption, decryption))
    })
  }
  ...
  async tryEncrypt(nonce: Uint8Array, block: Uint8Array, additionalData: Uint8Array): Promise<Result<Uint8Array, CryptoError>> {
    return await tryCrypto(async () => new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", length: 256, iv: nonce, additionalData }, this.encryption_key, block)))
  }

  async tryDecrypt(nonce: Uint8Array, block: Uint8Array, additionalData: Uint8Array): Promise<Result<Uint8Array, CryptoError>> {
    return await tryCrypto(async () => new Uint8Array(await crypto.subtle.decrypt({ name: "AES-GCM", length: 256, iv: nonce, additionalData }, this.decryption_key, block)))
  }
